...
public class EchoEffect implements EffectLayer {
    ...
    public void render(TooltipContext context, Vec2 position) {
        ...
        context.push(() -> {
            ...
            // Inner
            renderCore(tess, pose, centerX, centerY, maxRadius, time);

            // Rings
            renderPulses(tess, pose, centerX, centerY, maxRadius, time);
    ...
    private void renderCore(Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, float time) {
        ...
        bufferBuilder.addVertex(pose, centerX, centerY, 0)
                .setColor(ColorUtils.red(CORE_CENTER_COLOR), ColorUtils.green(CORE_CENTER_COLOR), ColorUtils.blue(CORE_CENTER_COLOR), centerA);
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            bufferBuilder.addVertex(pose, x, y, 0)
                    .setColor(ColorUtils.red(CORE_GLOW_COLOR), ColorUtils.green(CORE_GLOW_COLOR), ColorUtils.blue(CORE_GLOW_COLOR), edgeA);
        }
    ...
    private void renderPulses(Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, float time) {
        for (int i = 0; i < PULSE_COUNT; i++) {
            ...
            for (int seg = 0; seg <= SEGMENTS; seg++) {
                ...
                bufferBuilder.addVertex(pose, ox, oy, 0)
                        .setColor(ColorUtils.red(PULSE_COLOR_OUTER), ColorUtils.green(PULSE_COLOR_OUTER), ColorUtils.blue(PULSE_COLOR_OUTER), ringOuterA);

                bufferBuilder.addVertex(pose, ix, iy, 0)
                        .setColor(ColorUtils.red(PULSE_COLOR_INNER), ColorUtils.green(PULSE_COLOR_INNER), ColorUtils.blue(PULSE_COLOR_INNER), ringInnerA);
            }
