...
public class RimLightEffect implements EffectLayer {
    ...
    private static void draw(Matrix4f matrix4f, VertexConsumer vertexConsumer, Vec2 tooltipPos, Vec2 tooltipSize, float width, int color1, int color2, float padding) {
        ...
        // Left
        vertexConsumer.vertex(matrix4f, x0, y0, 0f).color(color1).endVertex();
        vertexConsumer.vertex(matrix4f, x0, y3, 0f).color(color1).endVertex();
        vertexConsumer.vertex(matrix4f, x1, y2, 0f).color(color2).endVertex();
        vertexConsumer.vertex(matrix4f, x1, y1, 0f).color(color2).endVertex();

        // Top
        vertexConsumer.vertex(matrix4f, x0, y0, 0f).color(color1).endVertex();
        vertexConsumer.vertex(matrix4f, x1, y1, 0f).color(color2).endVertex();
        vertexConsumer.vertex(matrix4f, x2, y1, 0f).color(color2).endVertex();
        vertexConsumer.vertex(matrix4f, x3, y0, 0f).color(color1).endVertex();

        // Right
        vertexConsumer.vertex(matrix4f, x2, y1, 0f).color(color2).endVertex();
        vertexConsumer.vertex(matrix4f, x2, y2, 0f).color(color2).endVertex();
        vertexConsumer.vertex(matrix4f, x3, y3, 0f).color(color1).endVertex();
        vertexConsumer.vertex(matrix4f, x3, y0, 0f).color(color1).endVertex();
