...
public class SonarEffect implements EffectLayer {
    ...
    private static void draw(Matrix4f pose, float cx, float cy, float innerR, float outerR, int r, int g, int b, int alphaPeak) {
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            buf.addVertex(pose, cx + function * midR, cy + sin * midR, 0)
                    .setColor(r, g, b, alphaPeak);
            buf.addVertex(pose, cx + function * innerR, cy + sin * innerR, 0)
                    .setColor(r, g, b, 0);
        }
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            buf2.addVertex(pose, cx + function * outerR, cy + sin * outerR, 0)
                    .setColor(r, g, b, 0);
            buf2.addVertex(pose, cx + function * midR, cy + sin * midR, 0)
                    .setColor(r, g, b, alphaPeak);
        }
