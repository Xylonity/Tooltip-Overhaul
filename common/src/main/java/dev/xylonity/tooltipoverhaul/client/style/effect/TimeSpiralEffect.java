...
public class TimeSpiralEffect implements EffectLayer {
    ...
    public void render(TooltipContext context, Vec2 position) {
        ...
        context.push(() -> {
            ...
            renderRadialHalo(tess, pose, centerX, centerY, maxRadius, tooltipWidth, tooltipHeight, time);

            renderOrbitRings(tess, pose, centerX, centerY, maxRadius, time, globalAngle);

            renderSpiralArms(tess, pose, centerX, centerY, maxRadius, baseThickness, time, globalAngle);
    ...
    private void renderRadialHalo(Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, int width, int height, float time) {
        ...
        for (int i = 0; i <= SEGMENTS; i++) {
            ...
            bufferBuilder.addVertex(pose, ox, oy, 0)
                    .setColor(edgeRed, edgeGreen, edgeBlue, 180);
            bufferBuilder.addVertex(pose, ix, iy, 0)
                    .setColor(coreRed, coreGreen, coreBlue, innerA);
        }
        ...
        for (int i = 0; i <= SEGMENTS; i++) {
            ...
            bufferBuilder2.addVertex(pose, ix, iy, 0)
                    .setColor(coreRed, coreGreen, coreBlue, innerA);
            bufferBuilder2.addVertex(pose, cx_pos, cy_pos, 0)
                    .setColor(coreRed, coreGreen, coreBlue, 0);
        }
    ...
    private void renderOrbitRings(Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, float time, float globalAngle) {
        ...
        for (int ringIndex = 0; ringIndex < 2; ringIndex++) {
            ...
            for (int i = 0; i <= SEGMENTS; i++) {
                ...
                bufferBuilder.addVertex(pose, ox, oy, 0)
                        .setColor(glowRed, glowGreen, glowBlue, haloAlpha);

                bufferBuilder.addVertex(pose, ix, iy, 0)
                        .setColor(ringRed, ringGreen, ringBlue, coreAlpha);
            }
    ...
    private void renderSpiralArms(Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, float baseThickness, float time, float globalAngle) {
        ...
        for (int arm = 0; arm < ARM_COUNT; arm++) {
            ...
            for (int i = 0; i <= SEGMENTS; i++) {
                ...
                bufferBuilder.addVertex(pose, outerX, outerY, 0)
                        .setColor(segR, segG, segB, outerAlpha);
                bufferBuilder.addVertex(pose, innerX, innerY, 0)
                        .setColor(segR, segG, segB, innerAlpha);
            }
