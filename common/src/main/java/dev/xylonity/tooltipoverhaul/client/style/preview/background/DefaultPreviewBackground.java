...
public class DefaultPreviewBackground implements PreviewBackgroundLayer {
    ...
    public void render(TooltipContext context, Vec2 startPosition, Vec2 endPosition) {
        ...
        // Top border
        context.getGraphics().fill(x1, y0 - 1, x0, y0, context.getLayerDepth().getZ(), backgroundColor);
        // Bottom border
        context.getGraphics().fill(x1, y1, x0, y1 + 1, context.getLayerDepth().getZ(), backgroundColor);
        // Left border
        context.getGraphics().fill(x1 - 1, y0, x1, y1, context.getLayerDepth().getZ(), backgroundColor);
        // Right border
        context.getGraphics().fill(x0, y0, x0 + 1, y1, context.getLayerDepth().getZ(), backgroundColor);
    }
