...
public class CrystalsEffect implements EffectLayer {
    ...
    public void render(TooltipContext context, Vec2 position) {
        ...
        context.push(() -> {
            ...
            for (int i = 0; i < CRYSTAL_COUNT; i++) {
                renderCrystal(bufferBuilder, pose, centerX, centerY, radiusX, radiusY, time, i);
            }
    ...
    private void renderCrystal(BufferBuilder bufferBuilder, Matrix4f pose, float centerX, float centerY, float radiusX, float radiusY, float time, int index) {
        ...
        for (int i = 0; i < outline.length; i++) {
            ...
            bufferBuilder.vertex(pose, x, y, 0)
                    .color(ColorUtils.red(highlightColor), ColorUtils.green(highlightColor), ColorUtils.blue(highlightColor), 235).endVertex();
            bufferBuilder.vertex(pose, x + a[0] * cos - a[1] * sin, y + a[0] * sin + a[1] * cos, 0)
                    .color(ColorUtils.red(facetColor), ColorUtils.green(facetColor), ColorUtils.blue(facetColor), 220).endVertex();
            bufferBuilder.vertex(pose, x + b[0] * cos - b[1] * sin, y + b[0] * sin + b[1] * cos, 0)
                    .color(ColorUtils.red(facetColor), ColorUtils.green(facetColor), ColorUtils.blue(facetColor), 220).endVertex();
        }
        ...
        bufferBuilder.vertex(pose, glintX, glintY - glintHalf, 0).color(ColorUtils.red(GLINT_COLOR), ColorUtils.green(GLINT_COLOR), ColorUtils.blue(GLINT_COLOR), glintAlpha).endVertex();
        bufferBuilder.vertex(pose, glintX + glintHalf, glintY, 0).color(ColorUtils.red(GLINT_COLOR), ColorUtils.green(GLINT_COLOR), ColorUtils.blue(GLINT_COLOR), glintAlpha).endVertex();
        bufferBuilder.vertex(pose, glintX, glintY + glintHalf, 0).color(ColorUtils.red(GLINT_COLOR), ColorUtils.green(GLINT_COLOR), ColorUtils.blue(GLINT_COLOR), glintAlpha).endVertex();
        bufferBuilder.vertex(pose, glintX - glintHalf, glintY, 0).color(ColorUtils.red(GLINT_COLOR), ColorUtils.green(GLINT_COLOR), ColorUtils.blue(GLINT_COLOR), glintAlpha).endVertex();
