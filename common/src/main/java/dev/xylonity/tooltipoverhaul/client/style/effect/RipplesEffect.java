        ...
        boolean updateAndRender(TooltipContext context, long nowMs) {
            ...
            private void draw(TooltipContext context, float centerX, float centerY, float innerR, float outerR, int color, float alphaPeak, float wobblePhase, long now) {
                ...
                for (int i = 0; i <= segments; i++) {
                    ...
                    buf.vertex(context.getPose().last().pose(), centerX + func2 * midR, centerY + func1 * midR, 0).color(red, green, blue, (int) (alphaNorm * 255f)).endVertex();
                    buf.vertex(context.getPose().last().pose(), centerX + func2 * innerR, centerY + func1 * innerR, 0).color(red, green, blue, 0).endVertex();
                }
                ...
                for (int i = 0; i <= segments; i++) {
                    ...
                    buf.vertex(context.getPose().last().pose(), centerX + func2 * outerR, centerY + func1 * outerR, 0).color(red, green, blue, 0).endVertex();
                    buf.vertex(context.getPose().last().pose(), centerX + func2 * midR, centerY + func1 * midR, 0).color(red, green, blue, (int) (alphaNorm * 255f)).endVertex();
                }
