...
public class NebulaEffect implements EffectLayer {
    ...
    public void render(TooltipContext context, Vec2 position) {
        ...
        context.push(() -> {
            ...
            renderNebulaLayers(bufferBuilder, tesselator, pose, centerX, centerY, maxRadius, tooltipWidth, tooltipHeight, time);

            renderSideWaves(bufferBuilder, tesselator, pose, centerX, centerY, maxRadius, time);

            renderParticles(bufferBuilder, tesselator, pose, positionX, positionY, tooltipWidth, tooltipHeight, centerX, centerY, maxRadius, time);
    ...
    private void renderNebulaLayers(BufferBuilder bufferBuilder, Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, int width, int height, float time) {
        ...
        for (int layer = 0; layer < 3; layer++) {
            ...
            for (int i = 0; i <= SEGMENTS; i++) {
                ...
                bufferBuilder.vertex(pose, ox, oy, 0)
                        .color(ColorUtils.red(color), ColorUtils.green(color), ColorUtils.blue(color), outerA)
                        .endVertex();
                bufferBuilder.vertex(pose, ix, iy, 0)
                        .color(ColorUtils.red(color), ColorUtils.green(color), ColorUtils.blue(color), innerA)
                        .endVertex();
            }
    ...
    private void renderSideWaves(BufferBuilder bufferBuilder, Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, float time) {
        ...
        for (int band = 0; band < 2; band++) {
            ...
            for (int i = 0; i <= SEGMENTS; i++) {
                ...
                bufferBuilder.vertex(pose, ox, oy, 0)
                        .color(ColorUtils.red(ENERGY_WAVE_COLOR), ColorUtils.green(ENERGY_WAVE_COLOR), ColorUtils.blue(ENERGY_WAVE_COLOR), edgeAlpha)
                        .endVertex();
                bufferBuilder.vertex(pose, ix, iy, 0)
                        .color(ColorUtils.red(ENERGY_WAVE_COLOR), ColorUtils.green(ENERGY_WAVE_COLOR), ColorUtils.blue(ENERGY_WAVE_COLOR), coreAlpha)
                        .endVertex();
            }
    ...
    private void renderParticles(BufferBuilder bufferBuilder, Tesselator tesselator, Matrix4f pose, int x, int y, int width, int height, float centerX, float centerY, float maxRadius, float time) {
