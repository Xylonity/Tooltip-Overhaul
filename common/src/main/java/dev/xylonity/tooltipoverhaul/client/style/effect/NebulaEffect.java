...
public class NebulaEffect implements EffectLayer {
    ...
    public void render(TooltipContext context, Vec2 position) {
        ...
        context.push(() -> {
            ...
            renderNebulaLayers(clip, bufferBuilder, tesselator, pose, centerX, centerY, maxRadius, tooltipWidth, tooltipHeight, time);

            renderSideWaves(clip, bufferBuilder, tesselator, pose, centerX, centerY, maxRadius, time);

            renderParticles(clip, bufferBuilder, tesselator, pose, positionX, positionY, tooltipWidth, tooltipHeight, centerX, centerY, maxRadius, time);
    ...
    private void renderNebulaLayers(EffectClip clip, BufferBuilder bufferBuilder, Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, int width, int height, float time) {
        ...
        for (int layer = 0; layer < 3; layer++) {
            ...
            for (int i = 0; i <= SEGMENTS; i++) {
                ...
                bufferBuilder.vertex(pose, ox, oy, 0)
                        .color(ColorUtils.red(color), ColorUtils.green(color), ColorUtils.blue(color), EffectRuntime.alpha(outerA))
                        .endVertex();
                bufferBuilder.vertex(pose, ix, iy, 0)
                        .color(ColorUtils.red(color), ColorUtils.green(color), ColorUtils.blue(color), EffectRuntime.alpha(innerA))
                        .endVertex();
            }

            clip.draw(bufferBuilder.end());
    ...
    private void renderSideWaves(EffectClip clip, BufferBuilder bufferBuilder, Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, float time) {
        ...
        for (int band = 0; band < 2; band++) {
            ...
            for (int i = 0; i <= SEGMENTS; i++) {
                ...
                bufferBuilder.vertex(pose, ox, oy, 0)
                        .color(ColorUtils.red(EffectRuntime.color(0, ENERGY_WAVE_COLOR)), ColorUtils.green(EffectRuntime.color(0, ENERGY_WAVE_COLOR)), ColorUtils.blue(EffectRuntime.color(0, ENERGY_WAVE_COLOR)), EffectRuntime.alpha(edgeAlpha))
                        .endVertex();
                bufferBuilder.vertex(pose, ix, iy, 0)
                        .color(ColorUtils.red(EffectRuntime.color(0, ENERGY_WAVE_COLOR)), ColorUtils.green(EffectRuntime.color(0, ENERGY_WAVE_COLOR)), ColorUtils.blue(EffectRuntime.color(0, ENERGY_WAVE_COLOR)), EffectRuntime.alpha(coreAlpha))
                        .endVertex();
            }
    ...
    private void renderParticles(EffectClip clip, BufferBuilder bufferBuilder, Tesselator tesselator, Matrix4f pose, int x, int y, int width, int height, float centerX, float centerY, float maxRadius, float time) {
        ...
        for (int i = 0; i < Math.round(PARTICLE_COUNT * EffectRuntime.density()); i++) {
            ...
            // Particle shape
            bufferBuilder.vertex(pose, px, py - half, 0)
                    .color(ColorUtils.red(color), ColorUtils.green(color), ColorUtils.blue(color), EffectRuntime.alpha(alpha))
                    .endVertex();
            bufferBuilder.vertex(pose, px + half, py, 0)
                    .color(ColorUtils.red(color), ColorUtils.green(color), ColorUtils.blue(color), EffectRuntime.alpha(alpha))
                    .endVertex();
            bufferBuilder.vertex(pose, px, py + half, 0)
                    .color(ColorUtils.red(color), ColorUtils.green(color), ColorUtils.blue(color), EffectRuntime.alpha(alpha))
