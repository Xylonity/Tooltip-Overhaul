...
public class FirefliesEffect implements EffectLayer {
    ...
    private void renderFirefly(BufferBuilder bufferBuilder, Matrix4f pose, float centerX, float centerY, float radiusX, float radiusY, float time, float[] firefly) {
        ...
        bufferBuilder.vertex(pose, x, y - half, 0).color(ColorUtils.red(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.green(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.blue(EffectRuntime.color(0, CORE_COLOR)), EffectRuntime.alpha(coreAlpha)).endVertex();
        bufferBuilder.vertex(pose, x + half, y, 0).color(ColorUtils.red(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.green(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.blue(EffectRuntime.color(0, CORE_COLOR)), EffectRuntime.alpha(coreAlpha)).endVertex();
        bufferBuilder.vertex(pose, x, y + half, 0).color(ColorUtils.red(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.green(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.blue(EffectRuntime.color(0, CORE_COLOR)), EffectRuntime.alpha(coreAlpha)).endVertex();
        bufferBuilder.vertex(pose, x - half, y, 0).color(ColorUtils.red(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.green(EffectRuntime.color(0, CORE_COLOR)), ColorUtils.blue(EffectRuntime.color(0, CORE_COLOR)), EffectRuntime.alpha(coreAlpha)).endVertex();
