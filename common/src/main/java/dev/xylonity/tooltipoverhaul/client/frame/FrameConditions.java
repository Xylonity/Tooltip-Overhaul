...
public record FrameConditions(
    ...
    public boolean matches(ItemStack stack) {
        ...
        if (hasCustomName != null && stack.hasCustomHoverName() != hasCustomName) {
            return false;
        }

        if (customName != null && (!stack.hasCustomHoverName() || !stack.getHoverName().getString().equals(customName))) {
            return false;
        }
        ...
        final Map<Enchantment, Integer> actual = EnchantmentHelper.getEnchantments(stack);
        if (enchanted != null && !actual.isEmpty() != enchanted) {
            return false;
        }

        for (final ResourceLocation id : enchantments) {
            if (!BuiltInRegistries.ENCHANTMENT.containsKey(id) || actual.getOrDefault(BuiltInRegistries.ENCHANTMENT.get(id), 0) <= 0) {
                return false;
            }
