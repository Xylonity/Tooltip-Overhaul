...
public class CustomFrameLoader {
    ...
    public static final ResourceLocation DEFAULT_SOURCE = new ResourceLocation(TooltipOverhaul.MOD_ID, CONFIG_SUBDIR + "/" + CONFIG_PATH);
            ...
            try {
                ...
                try (final Reader reader = source.openReader()) {
                    ...
                    for (final CustomFrameData frame : config.getCustomFrames()) {
                        frames.put(new ResourceLocation(source.location().getNamespace(), "rule/" + frames.size()), frame);
                    }
