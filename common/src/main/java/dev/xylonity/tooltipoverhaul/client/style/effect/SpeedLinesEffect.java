...
public class SpeedLinesEffect implements EffectLayer {
    ...
    public void render(TooltipContext context, Vec2 position) {
        ...
        context.push(() -> {
            ...
            renderWings(tesselator, pose, centerX, centerY, maxRadius, time);
    ...
    private void renderWings(Tesselator tesselator, Matrix4f pose, float centerX, float centerY, float maxRadius, float t) {
        ...
        for (int side = -1; side <= 1; side += 2) {
            ...
            for (int i = 0; i < RAYS_PER_SIDE; i++) {
                ...
                bufferBuilder.addVertex(pose, rootX, rootY, 0)
                        .setColor(innerRed, innerGreen, innerBlue, rootAlpha);
                bufferBuilder.addVertex(pose, tip1X, tip1Y, 0)
                        .setColor(outerRed, outerGreen, outerBlue, tipAlpha);
                bufferBuilder.addVertex(pose, tip2X, tip2Y, 0)
                        .setColor(outerRed, outerGreen, outerBlue, tipAlpha / 2 + rootAlpha / 2);
            }
