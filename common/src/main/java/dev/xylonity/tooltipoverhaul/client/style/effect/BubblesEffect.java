...
public class BubblesEffect implements EffectLayer {
    ...
    private void renderBubble(Tesselator tesselator, Matrix4f pose, int positionX, int positionY, int tooltipWidth, int tooltipHeight, float time, float[] bubble) {
        ...
        bufferBuilder.addVertex(pose, shineX, shineY, 0).setColor(ColorUtils.red(SHINE_COLOR), ColorUtils.green(SHINE_COLOR), ColorUtils.blue(SHINE_COLOR), shineAlpha);
        for (int i = 0; i <= 8; i++) {
            final float angle = (i / 8.0f) * (float) Math.PI * 2.0f;
            bufferBuilder.addVertex(pose, shineX + (float) Math.cos(angle) * shineRadius, shineY + (float) Math.sin(angle) * shineRadius, 0)
                    .setColor(ColorUtils.red(SHINE_COLOR), ColorUtils.green(SHINE_COLOR), ColorUtils.blue(SHINE_COLOR), 0);
        }
    ...
    private void ring(Tesselator tesselator, Matrix4f pose, float x, float y, float innerRadius, float midRadius, float outerRadius, int alpha) {
        // Inner half
        ...
        for (int i = 0; i <= RING_SEGMENTS; i++) {
            ...
            bufferBuilder.addVertex(pose, x + cos * midRadius, y + sin * midRadius, 0).setColor(ColorUtils.red(BUBBLE_COLOR), ColorUtils.green(BUBBLE_COLOR), ColorUtils.blue(BUBBLE_COLOR), alpha);
            bufferBuilder.addVertex(pose, x + cos * innerRadius, y + sin * innerRadius, 0).setColor(ColorUtils.red(BUBBLE_COLOR), ColorUtils.green(BUBBLE_COLOR), ColorUtils.blue(BUBBLE_COLOR), 0);
        }
        ...
        for (int i = 0; i <= RING_SEGMENTS; i++) {
            ...
            bufferBuilder.addVertex(pose, x + cos * outerRadius, y + sin * outerRadius, 0).setColor(ColorUtils.red(BUBBLE_COLOR), ColorUtils.green(BUBBLE_COLOR), ColorUtils.blue(BUBBLE_COLOR), 0);
            bufferBuilder.addVertex(pose, x + cos * midRadius, y + sin * midRadius, 0).setColor(ColorUtils.red(BUBBLE_COLOR), ColorUtils.green(BUBBLE_COLOR), ColorUtils.blue(BUBBLE_COLOR), alpha);
        }
